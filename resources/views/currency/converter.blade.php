        ...
        <div id="result" class="mt-6 md:mt-8 p-4 md:p-6 bg-gradient-to-br from-violet-300 to-violet-400 border border-violet-200 rounded-2xl shadow-xl hidden animate-fadeInUp">
            <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-3 md:gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-violet-100 mt-0.5 text-xs" id="resultInverse"></p>
                </div>
                ...
                <div class="flex flex-col items-start md:items-end gap-1 text-xs text-violet-50">
                    ...
                    <p>Rates are indicative and may differ from your bank.</p>
                    <p class="italic">Powered by ExchangeRate-API.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result && typeof result.result === 'number' && typeof result.rate === 'number') {
                ...
                const inverseRate         = result.rate > 0 ? (1 / result.rate).toFixed(4) : '—';
                resultInverse.textContent = `1 ${result.to} = ${inverseRate} ${result.from}`;
